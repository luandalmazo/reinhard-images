...
from sklearn.metrics import precision_score, f1_score, recall_score, confusion_matrix, roc_curve, auc, roc_auc_score
...
''' Test the model '''
def test(model, dataloader, device, class_names, mode):
    model.to(device)
    model.eval()
    y_true = []
    y_pred = []
    ...
    plot_roc_curve(y_true, np.array(probabilities), class_names, label)
...
def test_model(dataloader_test, classes, mode):
    ...
    for model_path in model_paths:
        ...
        precision, recall, f1, precision_class, recall_class, f1_class, = test(model, dataloader_test, device, class_names, mode)
